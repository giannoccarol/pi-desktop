...
function renderPackageStore() {
  ...
  for (const pkg of state.packages) {
    ...
    const tags = (pkg.types || []).map((type) => `<span class="package-type">${escapeHtml(type)}</span>`).join("") + (pkg.keywords || [])
      ...
      .join("");
    const rating = typeof pkg.rating==="number" ? pkg.rating : (pkg.score!=null ? Number(pkg.score) : null);
    const stars = rating!=null && rating>0 ? `<span title="Rating ${rating.toFixed(1)}">${"★".repeat(Math.round(rating))}${"☆".repeat(5-Math.round(rating))} ${rating.toFixed(1)}</span>` : (pkg.downloads>1000 ? `<span class="muted" title="Popolare">★ Popolare</span>` : "");
    card.innerHTML =
