...
(function exposeCosts(root){
  ...
  function openDashboard(){
    ...
    dlg.innerHTML = `<div class="modal-body" style="display:flex;flex-direction:column;gap:12px;max-height:80vh;overflow:auto"><div class="modal-title-row"><span class="modal-icon"><i data-lucide="bar-chart-3"></i></span><div class="grow"><h2>Costi — ${cwd.split("/").pop()||"progetto"}</h2><p class="muted small">${formatProjectCost(agg)} · ${sessions.filter(s=>s.cwd===cwd).length} sessioni</p></div><button class="icon-btn borderless" data-close><i data-lucide="x"></i></button></div>
      <div><strong style="font-size:12px">Ultimi 14 giorni (costo)</strong><div style="display:flex;gap:4px;align-items:flex-end;margin-top:6px">${tlBars||"<span class='muted small'>Nessun dato</span>"}</div></div>
      <div><strong style="font-size:12px">Breakdown provider/modello</strong><div style="margin-top:6px">${brRows}</div></div>
      <div style="display:flex;gap:6px;justify-content:flex-end"><button id="costs-csv" class="btn ghost small">Esporta CSV</button><button id="costs-deep" class="btn ghost small">Sessione più costosa →</button><button class="btn primary small" data-close>Chiudi</button></div></div>`;
