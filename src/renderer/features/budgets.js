...
(function exposeBudgets(root){
  ...
  const lastAlertLevel = new Map();

  function sessionsForBudget(cwd, budget, sessions){
    let selected=(sessions||[]).filter((session)=>session.cwd===cwd);
    if(budget.reset==="monthly"){
      const month=new Date().toISOString().slice(0,7);
      selected=selected.filter((session)=>String(session.timestamp || new Date(session.modified).toISOString()).slice(0,7)===month);
    }else if(budget.reset==="session") selected=selected.slice(0,1);
    return selected;
  }
  function aggregateBudget(cwd,budget,sessions){
    return root.piCosts?.getProjectCosts?.(cwd, sessionsForBudget(cwd,budget,sessions), null) || {count:0,cost:0,tokens:0};
  }
  ...
  function render(){
    ...
    for(const [cwd, b] of entries){
      const agg = aggregateBudget(cwd,b,s.sessions);
      ...
      row.querySelector("[data-edit]")?.addEventListener("click", async ()=>{
        ...
        if(maxCost!==null && (!Number.isFinite(maxCost)||maxCost<=0)) return toast("Il costo deve essere maggiore di zero","error");
        if(maxTokens!==null && (!Number.isFinite(maxTokens)||maxTokens<=0)) return toast("I token devono essere maggiori di zero","error");
        const next = {...budgets};
  ...
  function checkAlerts(){
    ...
    for(const [cwd,b] of Object.entries(budgets)){
      ...
      if(level && lastAlertLevel.get(cwd)!==level && pct<100){
        toast(`Budget ${cwd.split("/").pop()}: ${Math.round(pct)}% raggiunto`, "warn", 6000);
      } else if(level===100 && lastAlertLevel.get(cwd)!==100){
        toast(`Budget superato per ${cwd.split("/").pop()}!`, "error", 8000);
      }
      lastAlertLevel.set(cwd,level);
    }
  }
  function init(){
    const e = el();
