...
(function () {
  ...
  function wireUi() {
    ...
    try{
      ...
      el.costsBtn?.addEventListener("click", openCosts);
    }catch{}
    try{
      if(api && api.on){
        api.on("pi:event", (msg)=>{
    ...
    let searchRequest=0;
    const parseSessionSearch = (raw)=>{
      let text = String(raw || "").trim();
      const parsed = { raw:text, text:"", pinned:false, archived:false, project:"", tag:"", regex:null };
      text = text.replace(/(?:^|\s)pinned:(?:true)?(?=\s|$)/ig, ()=>{ parsed.pinned=true; return " "; });
      text = text.replace(/(?:^|\s)archived:(?:true)?(?=\s|$)/ig, ()=>{ parsed.archived=true; return " "; });
      text = text.replace(/(?:^|\s)(?:progetto|project):([^\s]+)/ig, (_m,value)=>{ parsed.project=String(value||"").toLowerCase(); return " "; });
      text = text.replace(/(?:^|\s)tag:([^\s]+)/ig, (_m,value)=>{ parsed.tag=String(value||"").toLowerCase(); return " "; });
      text = text.trim();
      if(text.startsWith("/") && text.lastIndexOf("/")>0){
        const end=text.lastIndexOf("/");
        const flags=text.slice(end+1);
        if(/^[imsu]*$/.test(flags)) try{ parsed.regex=new RegExp(text.slice(1,end), flags.includes("i")?flags:`${flags}i`); }catch{}
      }
      parsed.text=text;
      return parsed;
    };
    const doSearch = async ()=>{
      ...
      const parsed = parseSessionSearch(qRaw);
      const request = ++searchRequest;
      state.searchParsed = parsed;
      state.searchRegex = parsed.regex;
      if(mode==="fulltext" && (parsed.text.length>=2 || parsed.regex)){
        try{
          const query = parsed.regex ? {pattern:parsed.regex.source, flags:parsed.regex.flags} : parsed.text;
          const res = await api.searchFullText(query);
          if(request === searchRequest) state.searchFullTextResults = res;
        }catch{ if(request === searchRequest) state.searchFullTextResults = []; }
      } else {
        state.searchFullTextResults = [];
      }
      if(request === searchRequest) (window.renderProjects||window.piSidebar?.renderProjects)?.();
    };
    ...
    el.sessionSearch.addEventListener("keydown", (ev)=>{
      if(ev.key==="Escape"){ searchRequest++; el.sessionSearch.value=""; state.searchFullTextResults=[]; state.searchParsed=parseSessionSearch(""); state.searchRegex=null; (window.renderProjects||window.piSidebar?.renderProjects)?.(); }
    });
  ...
  async function boot() {
    ...
    state.terminalHistory = Array.isArray(state.settings?.terminalHistory) ? state.settings.terminalHistory.slice(-100) : [];
